...
int dimension = 45;
MT *mtree = NULL;
//string path = "E:\\EP\\DB\\Dimensionality & Radius\\NORMAL\\5-100.txt";
//string db = "E:\\EP\\INDEX\\Dimensionality & Radius\\NORMAL\\MTREE\\MTREE_5_100";
string path = "H:\\45-100.txt";
string db = "H:\\MTREE_45_10";
int amount = 100000;
...
void
CommandSelect (MTquery& query, int *gPages)
{
	int height = mtree->TreeHeight();
	int *pages = new int[height];
	for (int i=0; i<height; i++) {
		pages[i] = 0;
	}
	GiSTlist<MTentry *> list = mtree->RangeSearch(query, pages);
	...
	while (cmdLine != "quit") {
		...
		if (cmdLine == "drop") {
			...
			for (int i=0; i<amount/1000; i++) {
				...
				delete obj;
				SimpleQuery query(pred, 0);
				delete pred;
				CommandSelect (query, gPages);
			...
			//for (int i=height-1; i>=0; i--) {
			//	cout<<"Level "<<i<<": "<<((double)gPages[i])*1000/amount<<endl;
			//}
			delete[] gPages;
