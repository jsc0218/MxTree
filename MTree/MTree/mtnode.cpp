...
double MIN_UTIL;  // minimum node utilization [0, 1/2 or (n+1)/(2n)]
...
GiSTlist<MTentry *>
MTnode::RangeSearch (const MTquery &query, int *pages)
{
	if (pages) {
		++pages[this->Level()];
	}
	...
	if (IsLeaf()) {
		...
		for (int i=0; i<NumEntries(); i++) {
			...
			if (newQuery->Consistent(*entry)) {  // sub-tree included
				...
				GiSTlist<MTentry *> childResults = childNode->RangeSearch(*newQuery, pages);  // recurse the search
	...
	if (path.Level() > 1) {  // if we aren't in the root... 
		...
		if (parentEntry) {  // copy the entry
			...
			if (parentEntry->Key()->splitted) {  
				unionEntry->Key()->recomp = TRUE;
			}
			delete parentEntry;
		}
		if (unionEntry->Key()->distance < 0) {  // compute the distance from the parent
			MTnode *parentNode = ((MT *)Tree())->ParentNode((MTnode *)this);
			MTentry *grandEntry = parentNode->ParentEntry();

			if (unionEntry->Key()->distance == -MaxDist()) {
...
double MTnode::Overlap() const
{
	if (Level()==0 || NumEntries()==1) {
		return 0;
	}

	int counter = 0;
	for (int i=0; i<NumEntries(); i++) {
		MTentry *firstEntry = (MTentry *) (*this)[i].Ptr();
		for (int j=0; j<NumEntries(); j++) {
			if (i != j) {
				MTentry *secondEntry = (MTentry *) (*this)[j].Ptr();
				if (firstEntry->object().distance(secondEntry->object()) < firstEntry->MaxRadius()+secondEntry->MaxRadius()) {
					++counter;
					break;
				}
			}
	...
	return (float)counter/(float)NumEntries();
}
