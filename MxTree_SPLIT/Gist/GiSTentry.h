...
public:
	...
	virtual double operator = (const double d) { amount = d; return amount; }
...
public:
	GiSTentry (): level(0), node(NULL) {}
	GiSTentry (GiSTpage ptr): ptr(ptr), node(NULL) {}
	GiSTentry (const GiSTentry& e): ptr(e.ptr), level(e.level), node(e.node), position(e.position) {}
	virtual ~GiSTentry () {
		if (key) {
			delete key;
		}
...
protected:
	GiSTobject *key;  // The key is a pointer to a GiSTobject, which can be used to point to just about anything you like.
	GiSTpage ptr;  // a pointer to its child node
	int level;
	GiSTnode *node;  // this node
	int position;
};
