...
{
public:
	Object () { x = new int[dimension]; }  // default constructor (needed)
	...
	Object (char *buf) { x = new int[dimension]; memcpy (x, buf, dimension*sizeof(int)); }  // member constructor (needed)
	Object (int *buf) {  // member constructor (needed)
		x = new int[dimension];
		for (int i=0; i<dimension; i++) {
			x[i] = buf[i];
		}
	...
	double distance (const Object& other) const {  // distance method (needed)
		...
		for (int i=0; i<dimension; i++) {  // L2
			dist += pow ((double)(x[i]-other.x[i]), 2);
		}
		return sqrt(dist);
	}
	int CompressedLength () const { return ObjectSize (); }  // return the compressed size of this object
	void Compress (char *buf) { memcpy (buf, x, dimension*sizeof(int)); }  // object compression
