...
public:
	...
	virtual void Create (const char *filename);  // create a root node and write it to disk
	virtual void Open (const char *filename);  // just open the file, even create the file
	void Close ();  // just close the file

	GiSTcursor *Search (const GiSTpredicate &query) const;  // just new a cursor and return it
...
protected:
	...
	virtual GiSTlist<GiSTentry*> RemoveTop (GiSTnode *node);  // actually delete entries
	virtual double RemoveRatio () const { return 0.3; }

	// Reads/writes nodes
	virtual GiSTnode *ReadNode (const GiSTpath& path) const;
	virtual void WriteNode (GiSTnode *node);

//private:
	void InsertHelper (const GiSTentry &entry, int level, int *lvl_split=NULL);
	GiSTnode *ChooseSubtree (GiSTpage page, const GiSTentry &entry, int level);  // return a node suitable for insert
	virtual void Split (GiSTnode **node, const GiSTentry& entry);
	void AdjustKeys (GiSTnode *node, GiSTnode **parent);
	int CondenseTree (GiSTnode *leaf);
	void ShortenTree ();  // cut down the tree except the leftmost
