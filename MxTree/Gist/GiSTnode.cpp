...
GiSTnode::Expand (int index)
{
	...
	if (newMaxEntries < (index+1)) {  // not enough? adjust again!
		newMaxEntries = index + 1 + ALLOC_INCR;
	}

	GiSTentryptr *newEntries = new GiSTentryptr[newMaxEntries];
	for (int i=numEntries; i<newMaxEntries; i++) {
		newEntries[i] = NULL;
	}
	for (int i=0; i<numEntries; i++) {
...
GiSTnode::Pack (char *page) const
{
	...
	for (int i=0; i<numEntries; i++) {
		...
		if (compressedEntry.keyLen > 0) {
			memcpy (page+ltptr, compressedEntry.key, compressedEntry.keyLen);
		}
...
GiSTnode::SearchMinPenalty (const GiSTentry &newEntry) const
{
	...
	for (int i=0; i<numEntries; i++) {
		GiSTentry *e = (*this)[i];
		assert (e->Node() == this);
		GiSTpenalty *penalty = e->Penalty(newEntry);  // test
...
GiSTnode::PickSplit ()
{
	...
	node->numEntries = 0;
	for (int i=0; i<numEntries-half; i++) {
