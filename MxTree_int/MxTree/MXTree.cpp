...
GiSTnode* MXTree::ReadNode(const GiSTpath& path) const
{
	...
	int size = sizeof(MXTnodeHeader) + ((MXTnodeHeader *)firstBuffer)->numEntries*(EntrySize()+sizeof(GiSTpage));
	if (((MXTnodeHeader *)firstBuffer)->level == 0) {
		size += sizeof(int);
	}
	int pageNum = ceil((float)size/(float)PAGE_SIZE);
...
void MXTree::WriteNode(GiSTnode *node)
{
	...
	char *buffer = new char[store->PageSize()*pageNum];
	memset(buffer, 0, store->PageSize()*pageNum);
	((MXTnode *)node)->Pack(buffer);
	int startPage = (node->Path().IsRoot() ? rootPage : node->Path().Page());
	((MXTfile *)store)->Write(startPage, buffer, pageNum);  // do the deed
	delete[] buffer;
}

GiSTlist<MTentry *> MXTree::RangeSearch(const MTquery& query)
{
	GiSTpath path;
	path.MakeRoot();
	MXTnode *root = (MXTnode *)ReadNode(path);
